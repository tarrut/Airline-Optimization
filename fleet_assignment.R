...
solution_gen <- lp_solver(vars, T, T, T)
n_aircr <- sum(solution_gen$vars[var=="G"][i2 == par_ln,value])

schedule$`Flight time` <- round(as.integer(substr(schedule$`Flight time`, 1, 2)) + 
                                  as.integer(substr(schedule$`Flight time`, 4, 5))/60,1)

tb_fleet_assignment_1 <- data.table()
tb_fleet_assignment_1$`Flight no.` <- F_map$ref
tb_fleet_assignment_1$Fleet <- solution_gen$vars[value == 1][var == "x", .(K = i2)]

fwrite(tb_fleet_assignment_1, paste0("fleet_dist.csv"))

tb_fleet_assignment_1 <- merge(tb_fleet_assignment_1, K_map[, .(Fleet = ref, ind = as.character(ind))], by.x = "Fleet", by.y = "ind", sort = FALSE)
tb_fleet_assignment_1[, Fleet := NULL]
setnames(tb_fleet_assignment_1,"Fleet.y", "Fleet")


rm(list = setdiff(ls(), c("tb_fleet_assignment_1")))